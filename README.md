...
The app uses **Google Cloud Natural Language AI** to analyze the sentiment of user input, understanding how they feel, whether it's **very negative** or **very positive**. Based on this sentiment, it selects an appropriate response from a database of predefined feelings. The response is **randomized**, so it doesn't sound robotic or repetitive, making it feel like it's coming from a real AI. The goal is to make the interaction feel more **natural** and **human-like**.
...
* To use **Google Cloud Natural Language API**, you need to set up your **Google Cloud** account, enable the Natural Language API, and create a **service account**.
...
* By default, the app uses **SQLite** for simplicity. If you want to scale or need more flexibility, you can switch to **PostgreSQL** or any other relational database of your choice.
  ...
  2. Update the database URI in `app.py`:
     ...
     ```
  3. Set up your **PostgreSQL** database, create tables, and migrate the schema.
...
* The frontend is set to communicate with the backend running on **[http://localhost:5000](http://localhost:5000)**. If you're deploying the app, make sure to update this URL to the correct backend server.
